      ...
      <div v-if="activeTab === 'generation'" class="grid grid-cols-1 gap-3 lg:grid-cols-[260px_1fr_1fr]">
        <div class="card p-3">
          <ParamPanel v-bind="paramBindings" @update:resolutionTier="resolutionTier = $event" @update:selectedRatio="selectedRatio = $event"
            ...
            @update:outputCompression="outputCompression = $event" @update:background="background = $event" @update:stylePreset="stylePreset = $event" @update:imageCount="imageCount = $event" @update:qualityOverride="qualityOverride = $event" @update:upscaleMode="upscaleMode = $event" />
        </div>
