  ...
  <div @dragover.prevent="dragging = true" @dragleave="dragging = false" @drop.prevent="onDrop"
    ...
    <div v-else class="flex flex-wrap gap-1.5">
      <div v-for="(_f, i) in files" :key="i" class="group relative">
        <img v-if="previews[i]" :src="previews[i]" class="h-16 w-16 cursor-pointer rounded-lg object-cover" @click="openPreview(i)" />
        <div v-else class="flex h-16 w-16 items-center justify-center rounded-lg bg-gray-100 dark:bg-dark-700">
...
function openPreview(i: number) {
  if (previews.value[i]) {
    previewSrc.value = previews.value[i]
