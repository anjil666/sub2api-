...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	userService := service.NewUserService(userRepository, apiKeyAuthCacheInvalidator, billingCache, settingService)
	referralCommissionRepository := repository.NewReferralCommissionRepository(db)
	referralService := service.NewReferralService(userRepository, referralCommissionRepository, settingService, billingCache, apiKeyAuthCacheInvalidator)
	...
	auth401CounterCache := repository.NewAuth401CounterCache(redisClient)
	rateLimitService := service.ProvideRateLimitService(accountRepository, usageLogRepository, configConfig, geminiQuotaService, tempUnschedCache, timeoutCounterCache, auth401CounterCache, settingService, compositeTokenCacheInvalidator)
	...
	gatewayService.SetBackupService(backupService)
	...
	upstreamHandler := admin.NewUpstreamHandler(upstreamSiteRepository, upstreamSyncService)
	healthProbeConfigRepository := repository.NewHealthProbeConfigRepository(db)
