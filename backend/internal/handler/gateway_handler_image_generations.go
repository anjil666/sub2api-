...
// ImageGenerations handles OpenAI /v1/images/generations requests.
// Proxies the request to the upstream provider's /v1/images/generations endpoint.
func (h *GatewayHandler) ImageGenerations(c *gin.Context) {
	...
	if err := h.billingCacheService.CheckBillingEligibility(c.Request.Context(), apiKey.User, apiKey, apiKey.Group, subscription); err != nil {
		reqLog.Info("gateway.image_gen.billing_check_failed", zap.Error(err))
	...
	for {
		...
		result, err := h.gatewayService.ForwardAsImageGenerations(c.Request.Context(), c, account, body, requestStart)
		...
		if err != nil {
			...
			reqLog.Error("gateway.image_gen.forward_failed",
		...
		h.submitUsageRecordTask(func(ctx context.Context) {
			if err := h.gatewayService.RecordUsage(ctx, &service.RecordUsageInput{
				...
				reqLog.Error("gateway.image_gen.record_usage_failed",
					zap.Int64("account_id", account.ID),
					zap.Error(err),
				)
			}
